...
namespace DataAcess
{
    public class StudentDA : IStudentDA
    {
        ...
        {
            User user = await appContext.Users.AsNoTracking().FirstOrDefaultAsync(u => u.Login == username);
            if (user == null) return null;
            return await appContext.Students.AsNoTracking().FirstOrDefaultAsync(u => u.Id_user == user.ID);
        }
