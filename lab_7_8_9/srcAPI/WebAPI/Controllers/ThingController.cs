...
namespace WebAPI.Controllers
{
    ...
    public class ThingController : Controller
    {
        ...
        public async Task<IActionResult> getAllThing([FromQuery] string? login)
        {
            try
            {
                ...
                Student student = await studentServices.getStudentByUsername(login);
                List<Thing> res = new List<Thing>();
                foreach (Thing thing in allThings)
                    if (thing.Id_student == student.Id_student)
                        res.Add(thing);
